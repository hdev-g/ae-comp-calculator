...
export default async function DealsPage() {
  ...
  const totalAmount = deals.reduce((acc, d) => acc + decimalToNumber(d.amount), 0);
      ...
      <div className="mx-auto max-w-6xl">
        <div className="flex flex-col gap-6">
          <header className="flex flex-col gap-2">
            <div className="text-sm text-zinc-600">{role === "ADMIN" ? "All AEs" : "My deals"}</div>
            <h1 className="text-2xl font-semibold tracking-tight">Closed Won Deals</h1>
            <div className="text-sm text-zinc-600">
              {deals.length} deals • {formatCurrency(totalAmount)} total
            </div>
          </header>
                  ...
                  {deals.map((d) => {
                    ...
                    return (
                      <tr key={d.id} className="border-t border-zinc-100">
                        ...
                        {role === "ADMIN" ? (
                          <td className="px-5 py-4 text-zinc-700">{aeLabel}</td>
                        ) : null}
